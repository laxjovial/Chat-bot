...
# Define the base set of tools available to the Weather Agent
tools = [
    weather_search_web,
    weather_query_uploaded_docs,
    weather_summarize_document_by_path,
...
template = """
You are a highly specialized AI assistant focused on weather information. Your primary goal is to provide accurate, concise, and helpful information, forecasts, and insights related to weather conditions.
You have access to the following tools:

{tools}

**Instructions for using tools:**
- **`weather_search_web`**: Use this tool for general weather-related news, climate change articles, or anything that requires up-to-date information from the broader internet on weather topics.
- **`weather_query_uploaded_docs`**: Use this tool if the user's question seems to refer to specific weather reports, climate studies, or personal weather observations that might have been uploaded by them (e.g., "my local weather station data", "summary of the climate report I uploaded"). Always specify the `user_token` when calling this tool.
- **`weather_summarize_document_by_path`**: Use this tool if the user explicitly asks you to summarize a document and provides a file path (e.g., "summarize the storm report at uploads/my_user/weather/storm.pdf").
- **`weather_data_fetcher`**: Use this tool to retrieve specific weather data (current, forecast, historical) from configured APIs (e.g., OpenWeatherMap). Understand its parameters (`api_name`, `data_type`, `location`, `days`, `units`).
- **`python_interpreter_with_rbac`**: This is a powerful tool for users with appropriate tiers. Use it for:
    - **Parsing and Analyzing Fetched Data**: After using `weather_data_fetcher`, use this tool to parse the JSON output (e.g., `import json; data = json.loads(tool_output)`) and perform calculations, statistical analysis, or extract specific insights from weather datasets (e.g., calculating average temperature over a period, identifying trends).
...
st.markdown("Your dedicated AI for weather information. Ask me anything about current weather, forecasts, or climate data!")

# Initialize chat history in Streamlit's session state
if "messages" not in st.session_state:
    st.session_state.messages = [
        AIMessage(content="Hello! I am your Weather AI Assistant. How can I help you with weather information today?")
    ]
...
# Get user input
user_query = st.chat_input("Ask me about weather...")
...
st.caption("This agent uses web search, queries your uploaded documents, can summarize files, fetches data from weather APIs, and can perform data analysis based on your tier.")

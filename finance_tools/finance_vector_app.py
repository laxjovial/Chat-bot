...
from finance_tools.finance_tool import FINANCE_SECTION # Use FINANCE_SECTION constant
...
# Define the required tier for this specific page (Upload Finance Docs)
...
if not current_user:
    ...
    if not (user_tier and user_roles and (TIER_HIERARCHY.get(user_tier, -1) >= TIER_HIERARCHY.get(REQUIRED_TIER_FOR_THIS_PAGE, -1) or "admin" in user_roles)):
        st.error(f"🚫 Access Denied: Your current tier ({user_tier.capitalize()}) does not have access to upload Finance Documents. Please upgrade your plan to {REQUIRED_TIER_FOR_THIS_PAGE.capitalize()} or higher.")
        st.stop() # Halts execution
# --- End RBAC Access Check ---


# --- Streamlit UI ---
st.set_page_config(page_title="Upload Finance Docs", page_icon="⬆️", layout="centered")
st.title("Upload & Manage Finance Documents ⬆️")

st.markdown("Upload financial documents to enhance the AI Assistant's knowledge base. These documents will be indexed and made searchable via the 'Query Uploaded Finance Docs' app.")
...
if uploaded_file is not None:
    ...
    if st.button("Process Document"):
        with st.spinner("Processing and indexing document..."):
            try:
                # Call the generic process_upload with the specific finance section
                message = process_upload(uploaded_file, user_token, FINANCE_SECTION)
                st.success(message)
...
st.markdown("---")
st.header("Clear All Indexed Finance Data")
st.write("This will remove all uploaded files and indexed vector data for the finance section for your user. This action cannot be undone.")

if st.button("Clear All Finance Data", help="This action cannot be undone."):
    with st.spinner("Clearing data..."):
        try:
            # Call the generic clear_indexed_data with the specific finance section
            message = clear_indexed_data(user_token, FINANCE_SECTION)
            st.success(message)
        except Exception as e:
...
st.markdown("---")
st.subheader("Current Finance Data Status:")
upload_path_status = BASE_UPLOAD_DIR / user_token / FINANCE_SECTION
vector_path_status = BASE_VECTOR_DIR / user_token / FINANCE_SECTION

if upload_path_status.exists() and any(upload_path_status.iterdir()):
    st.info(f"📁 **Uploaded files exist** for finance in: `{upload_path_status}`")
else:
    st.info(f"No uploaded files found for finance.")

if vector_path_status.exists() and any(vector_path_status.iterdir()):
    st.info(f"🧠 **Indexed data exists** for finance in: `{vector_path_status}`")
else:
    st.info(f"No indexed data found for finance.")

st.markdown("---")
st.caption(f"Current User Token: `{current_user.get('user_id', 'N/A')}` (for demo purposes)")
st.caption("This app manages the documents that the 'Query Uploaded Finance Docs' app and the 'Finance AI Assistant' use.")

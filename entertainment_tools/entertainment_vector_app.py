...
from entertainment_tools.entertainment_tool import ENTERTAINMENT_SECTION # Use ENTERTAINMENT_SECTION constant
...
# --- Streamlit UI ---
st.set_page_config(page_title="Upload Entertainment Docs", page_icon="⬆️", layout="centered")
st.title("Upload & Manage Entertainment Documents ⬆️")

st.markdown("Upload documents related to entertainment to enhance the AI Assistant's knowledge base. These documents will be indexed and made searchable via the 'Query Uploaded Entertainment Docs' app.")
...
if uploaded_file is not None:
    ...
    if st.button("Process Document"):
        with st.spinner("Processing and indexing document..."):
            try:
                # Call the generic process_upload with the specific entertainment section
                message = process_upload(uploaded_file, user_token, ENTERTAINMENT_SECTION)
                st.success(message)
...
st.markdown("---")
st.header("Clear All Indexed Entertainment Data")
st.write("This will remove all uploaded files and indexed vector data for the entertainment section for your user. This action cannot be undone.")

if st.button("Clear All Entertainment Data", help="This action cannot be undone."):
    with st.spinner("Clearing data..."):
        try:
            # Call the generic clear_indexed_data with the specific entertainment section
            message = clear_indexed_data(user_token, ENTERTAINMENT_SECTION)
            st.success(message)
        except Exception as e:
...
st.markdown("---")
st.subheader("Current Entertainment Data Status:")
upload_path_status = BASE_UPLOAD_DIR / user_token / ENTERTAINMENT_SECTION
vector_path_status = BASE_VECTOR_DIR / user_token / ENTERTAINMENT_SECTION

if upload_path_status.exists() and any(upload_path_status.iterdir()):
    st.info(f"📁 **Uploaded files exist** for entertainment in: `{upload_path_status}`")
else:
    st.info(f"No uploaded files found for entertainment.")

if vector_path_status.exists() and any(vector_path_status.iterdir()):
    st.info(f"🧠 **Indexed data exists** for entertainment in: `{vector_path_status}`")
else:
    st.info(f"No indexed data found for entertainment.")

st.markdown("---")
st.caption(f"Current User Token: `{get_user_token()}` (for demo purposes)")
st.caption("This app manages the documents that the 'Query Uploaded Entertainment Docs' app and the 'Entertainment AI Assistant' use.")

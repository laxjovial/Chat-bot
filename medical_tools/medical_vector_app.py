...
from medical_tools.medical_tool import MEDICAL_SECTION # Use MEDICAL_SECTION constant
...
# Define the required tier for this specific page (Upload Medical Docs)
...
if not current_user:
    ...
    if not (user_tier and user_roles and (TIER_HIERARCHY.get(user_tier, -1) >= TIER_HIERARCHY.get(REQUIRED_TIER_FOR_THIS_PAGE, -1) or "admin" in user_roles)):
        st.error(f"🚫 Access Denied: Your current tier ({user_tier.capitalize()}) does not have access to upload Medical Documents. Please upgrade your plan to {REQUIRED_TIER_FOR_THIS_PAGE.capitalize()} or higher.")
        st.stop() # Halts execution
# --- End RBAC Access Check ---


# --- Streamlit UI ---
st.set_page_config(page_title="Upload Medical Docs", page_icon="⬆️", layout="centered")
st.title("Upload & Manage Medical Documents ⬆️")

st.markdown("Upload documents related to medical or health topics to enhance the AI Assistant's knowledge base. These documents will be indexed and made searchable via the 'Query Uploaded Medical Docs' app.")
...
if uploaded_file is not None:
    ...
    if st.button("Process Document"):
        with st.spinner("Processing and indexing document..."):
            try:
                # Call the generic process_upload with the specific medical section
                message = process_upload(uploaded_file, user_token, MEDICAL_SECTION)
                st.success(message)
...
st.markdown("---")
st.header("Clear All Indexed Medical Data")
st.write("This will remove all uploaded files and indexed vector data for the medical section for your user. This action cannot be undone.")

if st.button("Clear All Medical Data", help="This action cannot be undone."):
    with st.spinner("Clearing data..."):
        try:
            # Call the generic clear_indexed_data with the specific medical section
            message = clear_indexed_data(user_token, MEDICAL_SECTION)
            st.success(message)
        except Exception as e:
...
st.markdown("---")
st.subheader("Current Medical Data Status:")
upload_path_status = BASE_UPLOAD_DIR / user_token / MEDICAL_SECTION
vector_path_status = BASE_VECTOR_DIR / user_token / MEDICAL_SECTION

if upload_path_status.exists() and any(upload_path_status.iterdir()):
    st.info(f"📁 **Uploaded files exist** for medical in: `{upload_path_status}`")
else:
    st.info(f"No uploaded files found for medical.")

if vector_path_status.exists() and any(vector_path_status.iterdir()):
    st.info(f"🧠 **Indexed data exists** for medical in: `{vector_path_status}`")
else:
    st.info(f"No indexed data found for medical.")

st.markdown("---")
st.caption(f"Current User Token: `{current_user.get('user_id', 'N/A')}` (for demo purposes)")
st.caption("This app manages the documents that the 'Query Uploaded Medical Docs' app and the 'Medical AI Assistant' use.")

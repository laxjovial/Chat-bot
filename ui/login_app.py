        ...
        if st.button("🚀 Login", type="primary", use_container_width=True):
            if not username_or_email or not password:
                ...
                if user and verify_password(user.get("email"), password):
                    ...
                    success_alert, msg_alert = sender.send_security_alert(user.get("email"), "Successful login from new session")
                    if not success_alert:
                        st.warning(f"⚠️ Failed to send security alert: {msg_alert}")
    ...
    if st.button("🔓 Access Account", type="primary", use_container_width=True):
        if not token:
            ...
            if user:
                ...
                # Send security alert
                sender = EmailSender() # Instantiate EmailSender
                success_alert, msg_alert = sender.send_security_alert(user.get("email"), "Login via access token")
                if not success_alert:
                    st.warning(f"⚠️ Failed to send security alert: {msg_alert}")
...
# === Method 3: Email + OTP ===
    ...
    # Step 1: Send OTP
    # Condition to resend OTP or send for the first time
        ...
        if st.button("📨 Send OTP", type="secondary", use_container_width=True):
            if not email:
                ...
                if user:
                    ...
                    if otp:
                        sender = EmailSender() # Instantiate EmailSender
                        success_otp, msg_otp = sender.send_otp_email(email, otp)
                        if success_otp:
                            ...
                            st.error(f"❌ Failed to send OTP email: {msg_otp}. Please try again.")
                    else:
    ...
    if st.session_state.otp_sent and st.session_state.otp_email == email:
            ...
            if st.button("✅ Verify OTP", type="primary", use_container_width=True):
                if not user_otp:
                    ...
                    if verify_otp(email, user_otp):
                        ...
                        user = lookup_user_by_token(token)
                        sender = EmailSender() # Instantiate EmailSender
                        success_alert, msg_alert = sender.send_security_alert(email, "Login via OTP verification")
                        if not success_alert:
                            st.warning(f"⚠️ Failed to send security alert: {msg_alert}")
        ...
        with col2:
            if st.button("🔄 Resend OTP", use_container_width=True):
                # Reset OTP state to allow resending
                st.session_state.otp_sent = False
                st.session_state.otp_email = ""
                st.rerun() # Rerun to show "Send OTP" button again

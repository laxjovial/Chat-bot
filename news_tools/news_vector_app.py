...
from news_tools.news_tool import NEWS_SECTION # Use NEWS_SECTION constant
...
# --- Streamlit UI ---
st.set_page_config(page_title="Upload News Docs", page_icon="⬆️", layout="centered")
st.title("Upload & Manage News Documents ⬆️")

st.markdown("Upload news articles, research papers, or other documents to enhance the AI Assistant's knowledge base. These documents will be indexed and made searchable via the 'Query Uploaded News Docs' app.")
...
if uploaded_file is not None:
    ...
    if st.button("Process Document"):
        with st.spinner("Processing and indexing document..."):
            try:
                # Call the generic process_upload with the specific news section
                message = process_upload(uploaded_file, user_token, NEWS_SECTION)
                st.success(message)
...
st.markdown("---")
st.header("Clear All Indexed News Data")
st.write("This will remove all uploaded files and indexed vector data for the news section for your user. This action cannot be undone.")

if st.button("Clear All News Data", help="This action cannot be undone."):
    with st.spinner("Clearing data..."):
        try:
            # Call the generic clear_indexed_data with the specific news section
            message = clear_indexed_data(user_token, NEWS_SECTION)
            st.success(message)
        except Exception as e:
...
st.markdown("---")
st.subheader("Current News Data Status:")
upload_path_status = BASE_UPLOAD_DIR / user_token / NEWS_SECTION
vector_path_status = BASE_VECTOR_DIR / user_token / NEWS_SECTION

if upload_path_status.exists() and any(upload_path_status.iterdir()):
    st.info(f"📁 **Uploaded files exist** for news in: `{upload_path_status}`")
else:
    st.info(f"No uploaded files found for news.")

if vector_path_status.exists() and any(vector_path_status.iterdir()):
    st.info(f"🧠 **Indexed data exists** for news in: `{vector_path_status}`")
else:
    st.info(f"No indexed data found for news.")

st.markdown("---")
st.caption(f"Current User Token: `{get_user_token()}` (for demo purposes)")
st.caption("This app manages the documents that the 'Query Uploaded News Docs' app and the 'News AI Assistant' use.")
